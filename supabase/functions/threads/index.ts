...
serve(
    async (req: {
        ...
        url: string | URL;
        json: () => CreateThreadRequest | PromiseLike<CreateThreadRequest>;
    }) => {
        ...
        try {
            ...
            if (req.method === 'GET' && threadId) {
                ...
                if (isMessagesRequest) {
                    ...
                    const { data: thread, error: singleThreadError } = await supabaseClient
                        .from('threads')
                        .select('*')
                        .eq('id', threadId)
                        .eq('user_id', user.id)
            ...
            if (req.method === 'POST') {
                ...
                const { data: thread, error: createError } = await supabaseClient
                    .from('threads')
                    .insert({
                        title,
                        user_id: user.id,
            ...
            if (req.method === 'PUT' && threadId) {
                ...
                const { data: thread, error: updateError } = await supabaseClient
                    .from('threads')
            ...
            console.error('Threads function error:', functionError);
            return new Response(
                JSON.stringify({
